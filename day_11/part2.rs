...
fn main() {
    ...
    loop {
        ...
        let (x,y,b,z) = run_program(program.clone(), program_index, relative_base, colour);
        program = x.clone();
        program_index = y;
        relative_base = b;
    ...
    // Loop over a grid and print # for any white panels in that area
    for j in 0..6 {
        for i in 0..40 {
            let mut colour = 0;
            if panels.clone().contains_key(&(i,j)) {
                colour = *panels.get(&(i,j)).unwrap();
            }
            if colour == 1 {
                print!("#");
            } else {
...
fn run_program(mut list: Vec<i128>, mut index: i128, mut relative_base: i128, input: i128) -> (Vec<i128>, i128, i128, (i128,i128)) {
    let mut opcode: i128;
    let mut output = (0,0);
